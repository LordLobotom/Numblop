...
- The public version comes from `application/config/version` (currently `0.3.2`) and stays
...
- The Play version code (currently `10`) is pinned by the same test but moves on its own: every
