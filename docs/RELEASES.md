...
- The public version comes from `application/config/version` (currently `0.4.0`) and stays
...
- The Play version code (currently `12`) is pinned by the same test but moves on its own: every
