...
- The public version comes from `application/config/version` (currently `0.4.2`) and stays
...
- The Play version code (currently `14`) is pinned by the same test but moves on its own: every
...
- Privacy policy URL for Play Console (data safety + store listing):
  ...
  the site content is kept in step with; it is not itself served anywhere.
