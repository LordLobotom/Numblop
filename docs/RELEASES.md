...
- The public version comes from `application/config/version` (currently `0.4.4`) and stays
...
- The Play version code (currently `16`) is pinned by the same test but moves on its own: every
