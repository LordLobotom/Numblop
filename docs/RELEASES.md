...
- The public version comes from `application/config/version` (currently `0.2.4`) and stays
...
- The Play version code (currently `7`) is pinned by the same test but moves on its own: every
