...
Godot 4.6.2 + GDScript offline multiplication game for children. Portrait 390x844, EN/CS.
...
```powershell
# Full test suite (128 tests). -GodotPath is required from a .claude/worktrees/ checkout.
...
- Every user-facing string is a key in `localization/strings.csv` with non-empty `en` AND `cs`.
...
- Never trigger a real purchase/save from a test; `AppState` writes live save files.
